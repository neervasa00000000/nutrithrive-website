...
function writeCart(items) {
  if (window.Cart?.clear && window.Cart?.add) {
    window.Cart.clear();
    items.forEach((item) => window.Cart.add({ ...item, quantity: Number(item.qty || item.quantity || 1) }));
    updateCartCount();
...
window.NT = {
  ...
  addToCart(product, qty = 1) {
    ...
    if (window.Cart?.add) {
      window.Cart.add({ ...product, quantity: qty, weight: Number(product.weight || 0) });
      updateCartCount();
...
function bindHeader() {
  ...
  document.querySelectorAll("[data-qty-minus]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const inputEl = document.querySelector("[data-qty]");
      if (!inputEl) return;
...
const CONSENT_KEY = "nt-ds-v1-cookie-consent";
const OFFER_KEY = "nt-ds-v1-welcome-offer";
const SIGNUP_KEY = "nt-ds-v1-newsletter-signup";
...
function bindUnsubscribeForm() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    localStorage.setItem("nt-ds-v1-unsubscribe", JSON.stringify({ email: email.value.trim(), requestedAt: new Date().toISOString(), previewOnly: true }));
