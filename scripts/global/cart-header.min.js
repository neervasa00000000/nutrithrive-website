window.__NT_CART_HEADER_INITIALIZED__?console.log("[NutriThrive] cart-header.js already initialized, skipping duplicate load"):(window.__NT_CART_HEADER_INITIALIZED__=!0,(function(){function i(){const n=document.querySelector(".nav-links");if(!n){setTimeout(i,100);return}if(document.querySelector(".cart-icon-link"))return;const e=document.createElement("a");e.href="/cart",e.className="cart-icon-link",e.setAttribute("aria-label","View shopping cart"),e.style.cssText=`
            position: relative;
            display: flex;
            align-items: center;
            justify-content: center;
            text-decoration: none;
            color: #1a2e22;
            font-weight: 500;
            padding: 0.5rem;
            margin-left: 0.5rem;
        `,e.innerHTML=`
            <svg width="30" height="30" viewBox="0 0 25 25" fill="none" style="display: block;" aria-hidden="true">
                <path d="M8.5 10.5H5L6.5 19.5H18.5L20 10.5H16.5M8.5 10.5L10.2721 5.18377C10.4082 4.77543 10.7903 4.5 11.2208 4.5H13.7792C14.2097 4.5 14.5918 4.77543 14.7279 5.18377L16.5 10.5M8.5 10.5H16.5" stroke="#1a2e22" stroke-width="1.5"/>
                <path d="M12.5 10.5V19.5" stroke="#1a2e22" stroke-width="1.5"/>
                <path d="M9.5 19.5L8.5 10.5" stroke="#1a2e22" stroke-width="1.5"/>
                <path d="M15.5 19.5L16.5 10.5" stroke="#1a2e22" stroke-width="1.5"/>
                <path d="M19.5 13.5H5.5" stroke="#1a2e22" stroke-width="1.5"/>
                <path d="M19 16.5H6" stroke="#1a2e22" stroke-width="1.5"/>
            </svg>
            <span class="cart-badge" data-cart-count aria-label="items in cart" style="
                position: absolute;
                bottom: -4px;
                right: -4px;
                background: #175c36;
                color: white;
                font-size: 12px;
                font-weight: 700;
                display: none;
                align-items: center;
                justify-content: center;
                line-height: 1;
                min-width: 20px;
                height: 20px;
                border-radius: 50%;
                padding: 0 5px;
                box-sizing: border-box;
                box-shadow: 0 2px 8px rgba(0,0,0,0.2);
            ">0</span>
        `;const t=document.querySelector(".hamburger");t&&t.parentNode?t.parentNode.insertBefore(e,t):n.appendChild(e),window.Cart&&window.Cart.updateUI()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",i):i()})());
