...
FORM: User-pinned NutriThrive Design System v1.0 (Apple HIG as principles). Seed: user-pinned-ds-v1.
  ...
  <div class="wrap footer-grid">
    ...
    <div>
      ...
      <ul>
        ...
        <li><a href="${r.journal}">Journal</a></li>
        <li><a href="${r.faq}">FAQ</a></li>
        <li><a href="${r.contact}">Contact</a></li>
...
function homepage() {
  ...
  const liveSeo = LIVE_MODE ? extractSeo(path.join(ROOT, "index.html")) : null;
  return layout({
...
<section class="section" style="padding-top:0">
  <div class="wrap">
    ...
    <div class="journal-grid">
      <a class="article-card featured" href="/journal/what-does-moringa-powder-taste-like-honest-guide-2026/">
      ...
      <a class="article-card" href="/journal/how-to-choose-moringa-powder-australia-2026/">
      ...
      <a class="article-card" href="/journal/how-to-add-moringa-to-diet/">
...
function shopPage() {
  const liveSeo = LIVE_MODE ? extractSeo(path.join(ROOT, "products/index.html")) : null;
...
function aboutPage() {
  const liveSeo = LIVE_MODE ? extractSeo(path.join(ROOT, "pages/about/about.html")) : null;
  return layout({
...
function notFoundPage() {
  return layout({
    ...
    main: `<section class="not-found wrap-narrow">
      ...
      <form class="not-found-search" action="/journal/" method="get" role="search">
      ...
      <div class="not-found-actions"><a class="btn btn-primary" href="/">Go to homepage</a><a class="btn btn-secondary" href="/shop/">Shop products</a><a class="btn btn-secondary" href="/journal/">Browse every guide</a></div>
...
function journalIndex(articles, opts = {}) {
  ...
  const canonicalPath = opts.canonicalPath || "/journal";
  const articleCard = (article) => `<a class="article-card" href="/journal/${article.slug}/" data-journal-card data-journal-topic="${esc(journalTopic(article))}" data-search-text="${esc(`${stripTags(article.title)} ${stripTags(article.description)} ${article.category} ${journalTopic(article)} ${article.slug}`.toLowerCase())}">
  ...
  return layout({
    title: liveSeo?.title || "Moringa, Curry Leaves & Tea Journal | NutriThrive",
    ...
    extraHead:
      jsonLd(
        breadcrumbSchema([
          { name: "Home", item: `${LIVE}/` },
          { name: canonicalPath === "/blog/" ? "Blog" : "Journal", item: `${LIVE}${canonicalPath.replace(/\/$/, "") || "/journal"}` },
        ])
      ) +
      jsonLd({
        "@context": "https://schema.org",
        "@type": "CollectionPage",
        name: liveSeo?.title || "NutriThrive Journal",
    ...
    main: `
      <section class="page-intro wrap">
        ...
        <nav class="journal-topics" aria-label="Journal topics">
      ...
      <section class="section" style="padding-top:0">
        <div class="wrap">
          <a class="article-card featured journal-feature" href="/journal/${featured.slug}/" data-journal-feature data-search-text="${esc(`${stripTags(featured.title)} ${stripTags(featured.description)} ${featured.category} ${journalTopic(featured)} ${featured.slug}`.toLowerCase())}">
...
function extractArticleProse(slug, fallbackHtml) {
  ...
  if (!fs.existsSync(file)) return fallbackHtml;
...
function articlePage(meta, prose, allArticles, liveSeo = null) {
  ...
  const title = liveSeo?.title || humanCopy(stripTags(meta.title));
  ...
  ${jsonLd(
    breadcrumbSchema([
      { name: "Home", item: `${LIVE}/` },
      { name: "Journal", item: `${LIVE}/journal` },
      { name: title, item: url },
    ])
  )}`,
    main: `
      <nav class="wrap crumbs" aria-label="Breadcrumb">
        <a href="/">Home</a> / <a href="/journal/">Journal</a> / <span>${esc(title)}</span>
      </nav>
      <section class="wrap article-layout">
        <article>
          ...
          ${relatedArticles.length ? `<nav class="article-related" aria-labelledby="related-${esc(meta.slug)}">
            ...
            <ul>${relatedArticles.map((article) => `<li><a href="/journal/${article.slug}/">${esc(humanCopy(stripTags(article.title)))}</a></li>`).join("")}</ul>
          </nav>` : ""}
...
function articleImage(article) {
  ...
  if (source.startsWith("/assets/") && fs.existsSync(path.join(ROOT, source.slice(1)))) return source;
  if (source.startsWith("/ds-v1/") && fs.existsSync(path.join(ROOT, source.slice(1)))) return source.replace(/^\/ds-v1/, "");
  const topic = journalTopic(article);
...
function emit(rel, html, liveRel = rel) {
  writePage(LIVE_MODE ? liveRel : rel, html, LIVE_MODE ? ROOT : OUT);
}
...
function writeGeneratedJs(search) {
  ...
  if (LIVE_MODE) {
    ...
    fs.writeFileSync(path.join(ROOT, "scripts/global/ds-v1-search-index.js"), searchJs);
    fs.writeFileSync(path.join(ROOT, "scripts/global/ds-v1-catalog.js"), catalog);
...
function appendLiveRedirects() {
  const marker = "# === ds-v1 live routes (keep ranking /blog URLs) ===";
  const block = `${marker}
/shop /products/ 301
/shop/ /products/ 301
...
`;
  ...
  if (current.includes(marker)) {
    fs.writeFileSync(file, current.replace(new RegExp(`${marker}[\\s\\S]*$`), block));
  } else {
...
function main() {
  ...
  if (LIVE_MODE) {
    ...
    emit("journal/index.html", journalIndex(activeArticles));
    emit(
      "blog/index.html",
      journalIndex(activeArticles, {
        canonicalPath: "/blog/",
        seoFile: path.join(ROOT, "blog/index.html"),
      }),
      "blog/index.html"
    );
  ...
  console.log(`Wrote core pages + ${wrapped} journal articles to ds-v1/`);
  scanSeo();
}
