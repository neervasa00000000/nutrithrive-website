# Final site map — https://nutrithrive.com.au

**Generated:** 11 May 2026 (Australia)  
**Source of truth:** [`sitemap.xml`](../sitemap.xml) and [`audit/sitemap-urls.txt`](sitemap-urls.txt), produced by running `node scripts/build-sitemap.js` from the repository root. That script walks all HTML files, applies the [blocklist and noindex rules](../scripts/build-sitemap.js), and writes canonical `<loc>` URLs.

**Indexable URL count:** **55**

---

## 1. Homepage

| Public URL |
|------------|
| https://nutrithrive.com.au/ |

---

## 2. Core site pages

| Public URL |
|------------|
| https://nutrithrive.com.au/about |
| https://nutrithrive.com.au/contact |
| https://nutrithrive.com.au/faq |
| https://nutrithrive.com.au/privacy-policy |
| https://nutrithrive.com.au/pages/shipping/shipping-returns |
| https://nutrithrive.com.au/pages/newsletter/ |
| https://nutrithrive.com.au/melbourne/ |
| https://nutrithrive.com.au/buy-moringa-powder-australia/index.html |
| https://nutrithrive.com.au/nutrithrive_labs/ |

---

## 3. Products

| Public URL |
|------------|
| https://nutrithrive.com.au/products/ |
| https://nutrithrive.com.au/products/moringa-powder/ |
| https://nutrithrive.com.au/products/curry-leaves/ |
| https://nutrithrive.com.au/products/black-tea/ |
| https://nutrithrive.com.au/products/combo-pack/ |
| https://nutrithrive.com.au/products/moringa-soap/ |

---

## 4. Blog

**39 URLs:** `/blog/` index plus **38** article pages.

| Public URL |
|------------|
| https://nutrithrive.com.au/blog/ |
| https://nutrithrive.com.au/blog/10-dollar-superfood-replaced-200-supplement-stack-australia-2026.html |
| https://nutrithrive.com.au/blog/30-day-moringa-coffee-reset-australia-guide-2026.html |
| https://nutrithrive.com.au/blog/affordable-superfoods-organic-tax-marketplace-wellness-vic-nsw-qld-2026.html |
| https://nutrithrive.com.au/blog/best-protein-energy-bars-australia-2026-supermarket-guide.html |
| https://nutrithrive.com.au/blog/best-protein-powder-australia.html |
| https://nutrithrive.com.au/blog/best-superfoods-australia-comparison-health-conscious-adults.html |
| https://nutrithrive.com.au/blog/buy-moringa-powder-melbourne-victoria-chinese-guide.html |
| https://nutrithrive.com.au/blog/does-moringa-have-caffeine-truth-about-energy-focus-side-effects-2026.html |
| https://nutrithrive.com.au/blog/growing-moringa-australia-honest-frost-pots-2026.html |
| https://nutrithrive.com.au/blog/healthy-snack-melbourne-why-everyone-switching-to-moringa-2026.html |
| https://nutrithrive.com.au/blog/high-protein-moringa-recipes-australia-2026.html |
| https://nutrithrive.com.au/blog/honest-australians-guide-moringa-powder-2026.html |
| https://nutrithrive.com.au/blog/how-to-add-moringa-to-diet.html |
| https://nutrithrive.com.au/blog/how-to-read-moringa-batch-codes-freshness.html |
| https://nutrithrive.com.au/blog/how-to-use-moringa-powder-daily-without-the-bad-taste-2026.html |
| https://nutrithrive.com.au/blog/is-moringa-legit-what-science-and-real-users-say-2026.html |
| https://nutrithrive.com.au/blog/melbourne-body-burden-report-2026.html |
| https://nutrithrive.com.au/blog/melbourne-food-as-medicine-map-cafes-organic-stores-healthspan-eating-2026.html |
| https://nutrithrive.com.au/blog/moringa-brands-comparison-australia-2026.html |
| https://nutrithrive.com.au/blog/moringa-brands-comparison-australia-chinese-review.html |
| https://nutrithrive.com.au/blog/moringa-chemist-warehouse-vs-nutrithrive-quality-test-2025.html |
| https://nutrithrive.com.au/blog/moringa-melbourne-complete-growers-report-2026.html |
| https://nutrithrive.com.au/blog/moringa-melbourne-complete-guide-2026.html |
| https://nutrithrive.com.au/blog/moringa-powder-complete-buyers-guide-australia-2026.html |
| https://nutrithrive.com.au/blog/moringa-powder-guide-benefits-how-to-use-daily-where-to-buy-australia-2026.html |
| https://nutrithrive.com.au/blog/moringa-powder-victoria-seniors-joint-health.html |
| https://nutrithrive.com.au/blog/moringa-powder-vs-capsules-which-one-actually-works-better-2026.html |
| https://nutrithrive.com.au/blog/moringa-quality-test-shade-dried-vs-retail-australia-2026.html |
| https://nutrithrive.com.au/blog/moringa-vs-coffee-melbourne-energy-hack.html |
| https://nutrithrive.com.au/blog/moringa-vs-spirulina-vs-matcha-comparison-australia.html |
| https://nutrithrive.com.au/blog/nutrithrive-delivers-across-victoria-is-your-town-eligible-hint-yes.html |
| https://nutrithrive.com.au/blog/nutrithrive-dried-curry-leaves-tradition-health.html |
| https://nutrithrive.com.au/blog/plant-based-functional-foods-australia-wellness-nutrithrive-2026.html |
| https://nutrithrive.com.au/blog/rosabella-moringa-reviews-legit-or-overhyped-2026.html |
| https://nutrithrive.com.au/blog/science-shade-drying-vs-sun-drying-moringa.html |
| https://nutrithrive.com.au/blog/smart-moringa-daily-intake-australia-visual-guide-2026.html |
| https://nutrithrive.com.au/blog/where-to-buy-moringa-in-australia-online-vs-stores-2026-guide.html |
| https://nutrithrive.com.au/blog/why-i-built-nutrithrive-moringa-real-story-finest-moringa-powder.html |

---

## 5. HTML present in repo but omitted from `sitemap.xml`

These are **not** listed in `sitemap.xml` by design (see `PATH_BLOCKLIST`, thank-you flows, or `noindex` in `scripts/build-sitemap.js`). Run `INCLUDE_NOINDEX=1 node scripts/build-sitemap.js` to preview URLs that would appear if noindex were ignored (crawlers still respect page robots tags).

**Blocklisted paths (never in sitemap):**

- `tools/check-paypal-buttons.html`
- `scripts/moringa-melbourne-report-head.inc.html` (include fragment, not a standalone page)
- `pages/shop/cart.html`
- `pages/contact/thank-you.html`
- `pages/newsletter/thank-you.html`
- `pages/shop/thank-you.html`
- `pages/contact/index.html`
- `pages/products/index.html`
- `pages/homepage/index.html`
- `pages/benefits/moringa-benefits.html`
- `pages/melbourne-page.html`

**Skipped as `noindex` (labs tools and checkout payment):**

- `nutrithrive_labs/ai-text-detector.html`
- `nutrithrive_labs/converter.html`
- `nutrithrive_labs/dedup-lines.html`
- `nutrithrive_labs/password-generator.html`
- `nutrithrive_labs/pomodoro-timer.html`
- `nutrithrive_labs/quick-notes.html`
- `nutrithrive_labs/sitemap-generator.html`
- `nutrithrive_labs/time-converter.html`
- `pages/shop/payment.html`

**Exception:** `blog/index.html` is included in the sitemap despite noindex, per `INCLUDE_DESPITE_NOINDEX` in the build script.

---

## 6. Maintenance

- After adding or removing HTML pages, run **`node scripts/build-sitemap.js`** and commit `sitemap.xml` plus `audit/sitemap-urls.txt`.
- CI: [.github/workflows/sitemap.yml](../.github/workflows/sitemap.yml)

---

## 7. URL pattern notes

- Blog posts use **`.html`** under `/blog/`.
- Product URLs use **trailing slashes** (`/products/.../`).
- Clean routes (`/about`, `/contact`, `/faq`, and so on) map to files under `pages/` via your host configuration; the sitemap uses those public URLs, not raw `pages/.../*.html` paths.
